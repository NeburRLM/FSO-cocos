    ...
    for(int i=0; i<numFantasmes; i++)
		{
			...
			{
        sprintf(id_fantasmax, "%d", i);        
			  sprintf(f1_fx, "%d", f1[i].f);
			  sprintf(f1_cx, "%d", f1[i].c);
			  sprintf(f1_dx, "%d", f1[i].d);
			  sprintf(f1_rx, "%f", f1[i].r);

			 	execlp("./fantasma3", "fantasma3", id_taulerx, n_fil1x, n_colx, id_fantasmax, f1_fx, f1_cx, f1_dx, f1_rx, id_fi1c, id_fi2c, retardx,(char *)0);
